...
class GuessNumberParams(BaseModel, extra="forbid"):
    number: float

    @field_validator('number')
    @classmethod
    def validate_number(cls, v: float) -> float:
        if not (0.0 <= v <= 100.0):
            raise ValueError(f"Number must be between 0.0 and 100.0, got {v}")
        return v


class IteratedTwoThirdsAverageEnvironment(Environment):
    GAME_NAME = "Iterated Two-Thirds Average"
    BASE_ENV_ID = "IteratedTwoThirdsAverage-v0"
    VARIANTS = [
    ...
    def __init__(self, task_spec: JSONObject, secrets: dict[str, str] = {}) -> None:
        ...
        if not api_key:
            raise ValueError("openai_api_key required in secrets for IteratedTwoThirdsAverage (LLM opponent)")
    ...
    async def _get_opponent_action(self, observation: str, player_id: int) -> str:
        system_prompt = (
            f"You are playing Iterated Two-Thirds Average as Player {player_id}. "
            f"Guess a number between 0 and 100. "
            f"Respond with ONLY your number in brackets like [42.5]."
        )
        messages = [
    ...
    async def get_prompt(self) -> List[TextBlock]:
        ...
        prompt = (
            f"You are playing {self.GAME_NAME} against an opponent.\n\n"
            f"Rules:\n"
            f"- Each player guesses a number between 0 and 100.\n"
            f"- The winner is the player whose guess is closest to two-thirds of the average of all guesses.\n"
            f"- Think strategically about what your opponent might guess!\n\n"
            f"Use the guess_number tool to submit your guess.\n"
            f"For example: guess_number(number=33.3)\n\n"
            f"{obs_text}"
        )
    ...
    @tool
    async def guess_number(self, params: GuessNumberParams) -> ToolOutput:
        """Guess a number between 0.0 and 100.0. The winner is closest to 2/3 of the average."""
        if self.game_done:
            return ToolOutput(
